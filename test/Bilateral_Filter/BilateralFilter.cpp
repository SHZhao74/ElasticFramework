...
*/
//IplImage *image
BilateralFilter::BilateralFilter( double sigmaD, double sigmaR,int *dsize,int *rsize){
	int sigmaMax ;
	if(sigmaD > sigmaR)
		sigmaMax = sigmaD;
	else
	...
	clkernelD = new double[kernelSize*kernelSize];
	...
	*rsize = 256*256;
	...
	gaussSimilarity = new double[256*256];
	clkernelR = new double[256*256];
	for (int i = 0; i < 256*256; i++){
		gaussSimilarity[i] = exp((double)-((i)*(i) / twoSigmaRSquared));
