...
//	}
	...
	char data[1024];
	mem = clCreateBuffer(context, CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE, 1024, data, &errCode);
	if(errCode != CL_SUCCESS){
  ...
  const unsigned int work_dim = 1;
  size_t global_size[work_dim] = {1024};
  size_t local_size[work_dim] = {1};
	errCode = clEnqueueNDRangeKernel(queue, kernel, work_dim, NULL,
